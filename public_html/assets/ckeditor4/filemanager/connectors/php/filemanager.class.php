...
class Filemanager {
	...
	public function getinfo() {
		if (!$this->isValidPath($this->getFullPath())) {
			$this->error("No way.");
		}
	...
	public function rename() {
		...
		if (!nc_ckeditor_check_file_name($new_file)) {
	...
	public function add() {
		...
		if($this->config['upload']['fileSizeLimit'] > 0 && $_FILES['newfile']['size'] > ($this->config['upload']['fileSizeLimit'] * 1024 * 1024)) {
		...
		if (!nc_ckeditor_check_file_name($_FILES['newfile']['name'])) {
		    $this->error('Wrong file type');
                }
		...
		if (strtolower(pathinfo($_FILES['newfile']['name'], PATHINFO_EXTENSION)) === 'svg') {
			nc_sanitize_svg_file($current_path . $_FILES['newfile']['name']);
		}
	...
	public function preview($thumbnail) {
		...
		if (isset($this->get['path']) && file_exists($current_path)) {
			...
			header("Content-type: image/" . pathinfo($returned_path, PATHINFO_EXTENSION));
			header("Content-Transfer-Encoding: Binary");
			header("Content-length: " . filesize($returned_path));
			header('Content-Disposition: inline; filename="' . basename($returned_path) . '"');
                        
			// очищаем буфер вывода, чтобы не сломать thumbnail
			while (ob_get_level()) {
				ob_end_clean();
			}
			...
			$this->error(sprintf($this->lang('FILE_DOES_NOT_EXIST'), ' ' . basename($current_path)));
		}
	}
	...
	private function get_file_info($path='', $thumbnail = false) {
		...
		if(is_dir($current_path)) {
			...
			if($this->item['filetype'] == 'svg') {
				...
				$previewPath = $current_path;
				if($this->dynamic_fileroot != '') {
					$previewPath = $this->dynamic_fileroot . $current_path;
					$previewPath = preg_replace('~/+~', '/', $previewPath);
				}
...
private function getFullPath($path = '') {
	...
	if($this->config['options']['fileRoot'] !== false) {
                //NETCAT FIX
		$full_path = $this->doc_root . $this->formatPath(rawurldecode(str_replace ( $this->doc_root , '' , $path)));
                /*
...
private function isValidPath($path) {
	...
	return !strncmp(realpath($path), realpath($this->getFullPath()), strlen($this->getFullPath()));
}
 ...
 * @param string $path
 */
public function get_thumbnail($path,$recreate = false) {
	...
	if(!file_exists($thumbnail_fullpath) || $recreate == true) {

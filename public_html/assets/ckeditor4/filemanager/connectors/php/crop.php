...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $relPathPrefix = (string) ($config['rel_path'] ?? '');
    if ($relPathPrefix !== '' && strpos($decodedPath, $relPathPrefix . '/') === 0) {
        $decodedPath = substr($decodedPath, strlen($relPathPrefix));
    } elseif ($relPathPrefix !== '' && $decodedPath === $relPathPrefix) {
        $decodedPath = '/';
    }

    $img_src = rtrim($config['doc_root'], '/\\') . '/' . ltrim($decodedPath, '/');

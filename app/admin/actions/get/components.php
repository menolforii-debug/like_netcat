...
$tab = isset($_GET['tab']) ? (string) $_GET['tab'] : 'templates';
...
function renderComponentsBlock(array $ctx, bool $wrap): void
{
    ...
    echo '<div class="accordion" id="componentsTree">';
    foreach ($components as $component) {
        $id = (int) $component['id'];
        ...
        $collapseId = 'componentCollapse' . $id;
        $headerId = 'componentHeading' . $id;
        ...
        echo '<span class="text-truncate">' . htmlspecialchars($name, ENT_QUOTES, 'UTF-8') . '</span>';
        echo '<span class="ms-2 text-muted small">' . htmlspecialchars($keyword, ENT_QUOTES, 'UTF-8') . '</span>';
        echo '</button>';
        echo '</h2>';

        echo '<div id="' . htmlspecialchars($collapseId, ENT_QUOTES, 'UTF-8') . '" class="accordion-collapse collapse' . $showClass . '" aria-labelledby="' . htmlspecialchars($headerId, ENT_QUOTES, 'UTF-8') . '" data-bs-parent="#componentsTree">';
        ...
        echo '<div class="d-flex align-items-center justify-content-between mb-2">';
        echo '<div class="fw-semibold small">Формы</div>';
        echo '<a class="btn btn-icon-square btn-outline-primary" href="' . htmlspecialchars($addViewLink, ENT_QUOTES, 'UTF-8') . '" title="Добавить форму" aria-label="Добавить форму">+</a>';
        echo '</div>';

        echo '<div class="list-group list-group-flush mb-2">';
        ...
        echo '<a class="btn btn-sm btn-outline-secondary" href="' . htmlspecialchars($settingsLink, ENT_QUOTES, 'UTF-8') . '">Настройки</a>';
    ...
    foreach (['general' => 'Общее', 'fields' => 'Поля', 'templates' => 'Формы'] as $key => $label) {
        $active = $tab === $key ? ' active' : '';
        $link = buildAdminUrl([
            'action' => 'components',
            'component_id' => (int) $selectedComponent['id'],
            'tab' => $key,
            'view' => $selectedView !== '' ? $selectedView : null,
        ]);
    ...
    if ($tab === 'templates') {
        ...
        if ($addView === '1') {
            ...
            echo csrf_token_field();
            echo '<input type="hidden" name="component_id" value="' . (int) $selectedComponent['id'] . '">';
            echo '<div class="mb-3"><label class="form-label">Название формы</label><input class="form-control" name="view" required></div>';
            echo '<button class="btn btn-primary" type="submit">Создать форму</button>';
            echo '</form>';
        } else {
            ...
            echo '<label class="form-label">list.php</label>';
            echo '<textarea class="form-control font-monospace" id="code_list" name="list_content" rows="10">' . renderTextareaValue($templateContents['list']) . '</textarea>';
            ...
            echo '<div class="mb-3 js-code-editor-wrapper">';
            echo '<label class="form-label">single.php</label>';
            echo '<textarea class="form-control font-monospace" id="code_single" name="single_content" rows="10">' . renderTextareaValue($templateContents['single']) . '</textarea>';
            echo '<div class="mt-2 d-flex gap-2">';
            echo '<button class="btn btn-link p-0 link-dotted js-code-editor-expand" type="button">Развернуть</button>';
            echo '<button class="btn btn-link p-0 link-dotted js-code-editor-collapse d-none" type="button">Свернуть</button>';
            echo '</div>';
            ...
            echo '<div class="mb-3 js-code-editor-wrapper">';
            echo '<label class="form-label">system.php</label>';
            echo '<textarea class="form-control font-monospace" id="code_system" name="system_content" rows="10">' . renderTextareaValue($templateContents['system']) . '</textarea>';
            echo '<div class="mt-2 d-flex gap-2">';
            echo '<button class="btn btn-link p-0 link-dotted js-code-editor-expand" type="button">Развернуть</button>';
            echo '<button class="btn btn-link p-0 link-dotted js-code-editor-collapse d-none" type="button">Свернуть</button>';
            echo '</div>';
            ...
            echo '<form method="post" action="/admin.php?action=component_view_delete" onsubmit="return confirm(\'Удалить форму и файлы шаблонов?\')">';
            echo csrf_token_field();
            echo '<input type="hidden" name="component_id" value="' . (int) $selectedComponent['id'] . '">';
            echo '<input type="hidden" name="view" value="' . htmlspecialchars($selectedView, ENT_QUOTES, 'UTF-8') . '">';

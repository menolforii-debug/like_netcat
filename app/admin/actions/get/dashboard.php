    ...
    if ($selected !== null) {
        ...
        if ($isAdmin) {
            ...
            if ($siteTab === 'settings') {
                ...
                echo '</select><div class="form-text">Наследуется разделами, если у них не задан собственный макет (например, «Макет1_внутри»).</div></div>';
            } else {
        ...
        if ($tab === 'section') {
            ...
            if ($isAdmin) {
                ...
                if ($sectionTab === 'settings') {
                    ...
                    echo '</select><div class="form-text">Наследует настройки сайта, можно задать отдельный шаблон (например, «Макет1_главная»).</div></div>';
                } else {
                    echo '<input type="hidden" name="title" value="' . htmlspecialchars((string) ($selected['title'] ?? ''), ENT_QUOTES, 'UTF-8') . '">';

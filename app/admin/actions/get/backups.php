...
if ($restoreLogEntries === []) {
    ...
    foreach ($restoreLogEntries as $entry) {
        ...
        $statusMap = [
            ...
            'strict_rejected' => 'отклонено strict-политикой',
            'preflight_failed' => 'ошибка preflight',
            'apply_failed_rolled_back' => 'ошибка применения, выполнен откат',
            'postcheck_failed' => 'ошибка post-check, выполнен откат',
        ];
        ...
        if ($statusRaw === 'success') {
            $statusClass = 'success';
        } elseif (str_contains($statusRaw, 'failed')) {
            $statusClass = 'danger';
        } elseif ($statusRaw === 'dry_run' || $statusRaw === 'strict_rejected') {
            $statusClass = 'warning';
        }
...
echo '<div class="fw-semibold mb-1">Как работают режимы восстановления</div>';
echo '<ul class="mb-0 ps-3">';
echo '<li><strong>Строгая</strong>: архив должен содержать все целевые пути бэкапа. Если чего-то нет — восстановление отменяется.</li>';
echo '<li><strong>Мягкая</strong>: допускается неполный архив. Восстанавливаются только пути, которые реально есть в архиве.</li>';
echo '<li><strong>Только проверка (dry-run)</strong>: показывает план действий без изменений на диске.</li>';
echo '</ul>';
echo '</div>';
...
if ($backupFiles === []) {
    ...
    foreach ($backupFiles as $file) {
        ...
        echo '</form>';
        echo '<form class="d-inline" method="post" action="/admin.php?action=backups_delete" onsubmit="return confirm(\'Удалить этот файл бэкапа без возможности восстановления?\');">';
        echo csrf_token_field();
        echo '<input type="hidden" name="file" value="' . htmlspecialchars($name, ENT_QUOTES, 'UTF-8') . '">';

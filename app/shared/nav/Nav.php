...
final class Nav
{
    ...
    public function get(): array
    {
        ...
        $items = array_filter($items, function (array $item): bool {
            ...
            $id = (int) ($item['id'] ?? 0);
            return $this->resolveShowInMenu($id);
        });
    ...
    private function mapItem(array $section): array
    {
        ...
        if (!empty($extra['menu_title']) && is_string($extra['menu_title'])) {
            $name = (string) $extra['menu_title'];
        }
        $parentId = isset($section['parent_id']) ? (int) $section['parent_id'] : 0;
    ...
    private function resolveShowInMenu(int $sectionId): bool
    {
        if ($sectionId <= 0) {
            return true;
        }
        ...
        while ($currentId > 0 && !isset($visited[$currentId])) {
            $visited[$currentId] = true;
            if (!isset($this->byId[$currentId])) {
                break;
            }
            $extra = Utils::decodeExtra($this->byId[$currentId]);
            $inherit = !array_key_exists('show_in_menu_inherit', $extra) || !empty($extra['show_in_menu_inherit']);
            $show = !array_key_exists('show_in_menu', $extra) || !empty($extra['show_in_menu']);
            if (!$inherit) {
                return $show;
            }
            $parentId = $this->parentById[$currentId] ?? 0;
            if ($parentId <= 0 || $parentId === $currentId) {
                break;
            }

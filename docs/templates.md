...
В админке для представления компонента доступно поле **«Системные настройки»** (`component_views.system_tpl`).
Код из этого поля вставляется в начало сгенерированного шаблона и позволяет объявлять вспомогательные функции,
переменные или переопределять данные перед выводом list/single.

...
В шаблонах компонентов можно вызывать `objects_list()` для выборки объектов по фильтрам
(см. подробное описание в `docs/objects-api.md`).

Также доступна функция `resize_image($sourcePath, $maxWidth, $maxHeight, $destPath = null, $quality = 85)`,
...
Если требуется влиять на выборку, используйте собственные запросы или `objects_list()` прямо

...
- `insert_snip($keyword)` — вставляет содержимое врезки по ключу и выполняет PHP-код внутри неё (сырое HTML/JS/PHP из админки, без экранирования).
...
Полный список функций для макетов, компонентов и врезок см. в `docs/help.md`.

...
`resolveTemplatePath(string $componentKey, string $view, bool $isSingle): string` возвращает путь к шаблону компонента (`list.php` или `single.php`) и может использоваться в макетах/обёртках, где требуется подставить файл шаблона по ключу компонента и имени представления.

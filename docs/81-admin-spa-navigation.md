...
### Примечание про adminOk()

`adminOk()` больше не добавляет `refresh` автоматически: если нужен selector-refresh,
его нужно указывать явно в месте вызова. Это помогает не размывать контракт ответа.
...
- Не актуально: ранее `app/admin/actions/post/object_update.php` возвращал `refresh` в JSON-ответе,
  теперь AJAX-ветка отвечает `{"ok": true}` без selector-refresh. Раздел оставлен как напоминание
  о типе ошибок, но конкретное отклонение исправлено.

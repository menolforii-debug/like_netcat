...
`GET`-страница макетов использует HTML-партиалы:
- `renderLayoutsSidebarHtml()` — рендер списка макетов (часть сайдбара).
- `renderLayoutsContentHtml()` — рендер основной формы и вкладок.
- `renderVisualFieldsContent()` — рендер блока визуальных полей на вкладке визуальных настроек.

Партиалы обрабатываются до основной логики страницы: при `AJAX`-обновлении
используйте `partial=sidebar|content|visual_fields`, чтобы вернуть только HTML блока.
...
Для пользовательского фидбэка в `AJAX`-режиме возвращайте `jsonResponse()` с `message`,
`focus` и `refresh`, чтобы UI обновился без полной перезагрузки.

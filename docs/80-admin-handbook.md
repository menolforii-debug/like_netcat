...
`componentKeyIsValid()`, `snippetKeyIsValid()` и `layoutKeyIsValid()` сейчас повторяют один и тот же
набор проверок (regex + защита от `..`, `/`, `\`). Это потенциальный источник рассинхронизации,
поэтому при изменении правил безопаснее вынести общую проверку в единый хелпер и использовать
его внутри всех трёх функций.

...
- политика «Строгая» (`strict`) — запрещает восстановление из неполного архива;
- политика «Мягкая» (`lenient`) — разрешает неполный архив и очищает только цели, которые есть в архиве.

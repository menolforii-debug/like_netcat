...
### Дублирование рендера file input + preview
В `renderFieldInput()` есть сборка блока с file input, превью, кнопкой очистки и чекбоксом
удаления, при этом похожая разметка повторяется в `app/admin/actions/get/dashboard.php` для
визуальных настроек. Это дублирование стоит убрать: либо вынести общий хелпер для рендера
файлового поля, либо переиспользовать `renderFieldInput()` там, где это уместно.

...
`collectSections()` и `collectSectionTree()` выполняют один и тот же рекурсивный обход дерева
через `SectionRepo::listChildren()` и отличаются только тем, что вторая функция добавляет поле
`depth` в каждый узел. При рефакторинге стоит рассмотреть единый обход с опциональным
добавлением глубины, чтобы не поддерживать две параллельные реализации.

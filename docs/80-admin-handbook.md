...
- восстановление/проверка выбранного архива (`POST /admin.php?action=backups_restore`, режимы: восстановление или dry-run);
- удаление выбранного архива (`POST /admin.php?action=backups_delete`).

Восстановление выполняется в несколько шагов предосторожности: preflight-проверка состава архива,
...
- режим «Только проверка (dry-run)» — предпросмотр плана без изменений на диске;
- режим «Восстановить» — фактическое применение бэкапа;
- политика «Строгая» (`strict`) — архив обязан содержать **все** целевые пути бэкапа; если хотя бы одного нет, операция останавливается до начала изменений;
- политика «Мягкая» (`lenient`) — допускает неполный архив: очищаются/восстанавливаются только те цели, которые реально присутствуют в архиве;
- практическое правило: `strict` для штатных полных бэкапов, `lenient` — для частичных/миграционных архивов, когда вы осознанно не трогаете отсутствующие цели.

Ниже списка бэкапов отображается «Журнал восстановлений» (последние 30 записей из `var/logs/backup-restore.log`)
со статусами операций для быстрого операционного контроля из UI.
Если maintenance lock завис, вверху страницы показывается карточка lock-состояния и кнопка «Снять lock вручную» (POST `backups_unlock`).

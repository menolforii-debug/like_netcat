...
При отправке форм с `data-ajax="true"` действия возвращают JSON:
- `message` — текст успешной операции;
- `error` — текст ошибки;
- `refresh` — селекторы для обновления списка и формы;
- `focus.keyword` — синхронизация URL с выбранной врезкой.

`GET`-страница врезок рендерит блоки через:
- `renderSnippetsSidebarHtml()` — список врезок в левой колонке;
- `renderSnippetsContentHtml()` — форма редактирования/создания.

Их можно запрашивать отдельно через `partial=sidebar|content` в `AJAX`-режиме.
